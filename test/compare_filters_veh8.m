% =========================================================================
% compare_filters.m
% 比较 EKF,PF,UKF,IEKF,SE(2)-DMLKF 在多车协同定位下的表现 (8车升级版)
% 功能：
% 1. 同步运行五种滤波器 (加入 IEKF)
% 2. 绘制 8 辆车误差对比子图
% 3. 命令行输出各车辆的 RMSE 对比
...
if ~exist(data_path, 'file')
    error('未找到数据文件，请先运行 trajectory_Veh8.m');
end
load(data_path); 

save_result_dir = 'E:\SE2_DMLKF_Project\Result\Veh8';
if ~exist(save_result_dir, 'dir')
    mkdir(save_result_dir);
end
result_full_path = fullfile(save_result_dir, 'Filter_Results_Veh8.mat');

if_read_result = 1; % 1 读取已有结果作图，0 重新生成新的结果保存
...
if ~if_read_result
    ...
    % 执行保存
    save(result_full_path, 'filter_results_Veh8');
    fprintf('==== 结果保存完成！====\n');
end
...
sgtitle('Horizontal Position Error Comparison (8 Vehicles)', 'FontSize', 14, 'FontWeight', 'bold');
for i = 1:num_vehicles
    subplot(num_vehicles, 1, i); % 动态适配 8 行画布
